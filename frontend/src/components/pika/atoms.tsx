...
// ── Logo mark ──────────────────────────────────────────────
interface PikaMarkProps {
  size?: number;
  color?: string;
}

export function PikaMark({ size = 32, color = '#fff' }: PikaMarkProps) {
  return (
...
export function PikaWordmark({ height = 36, color = '#fff', subdued = false }: PikaWordmarkProps) {
  return (
...
export function Confetti({ seed = 7, density = 70 }: ConfettiProps) {
...
export function Avatar({ name = '', size = 44, src = null, ring = false }: AvatarProps) {
  ...
  if (src) return <img src={src} style={{ ...style, objectFit: 'cover' }} alt={name} />;
  return <div style={style}>{initials}</div>;
}

// ── Money formatter (MXN by default) ─────────────────────
export function fmtMXN(n: number, { sign = false } = {}) {
